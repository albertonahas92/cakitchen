...
class User extends Public_Controller
{
    ...
    private function send($array)
    {
        ...
        $send = array('csrf_token' => $this->security->get_csrf_hash()) + $array;
...
print_r($this->input->post());
        print_r($_POST[0]);
        echo "emplo".implode(" ",$_POST);
       echo "data";
echo("ppp . ".$this->input->post("password"));
echo $_POST["password"];
//echo "heehheeeeedddddeehhee";
        // validators
        //    $this->form_validation->set_error_delimiters($this->config->item('error_delimeter_left'), $this->config->item('error_delimeter_right'));
        // $this->form_validation->set_rules('username', lang('users input username'), 'required|trim|min_length[5]|max_length[30]|callback__check_username');
        //   $this->form_validation->set_rules('first_name', lang('users input first_name'), 'required|trim|min_length[2]|max_length[32]');
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            // save the changes
            $validation_code = $this->users_model->create_profile($_POST);
